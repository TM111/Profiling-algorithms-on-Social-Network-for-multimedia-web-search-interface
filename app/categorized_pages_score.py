    ...
    for i in range(0,len(tmp_A)):
        for a in A:
            if tmp_A[i][cat]==a[cat]:
                tmp_A[i]={'name':tmp_A[i]['name'],cat:tmp_A[i][cat],'fan_count':tmp_A[i]['fan_count'],'likes':tmp_A[i]['likes']+1}
    
    for i in range(0,len(tmp_B)):
        for b in B:
            if tmp_B[i][cat]==b[cat]:
                tmp_B[i]={'name':tmp_B[i]['name'],cat:tmp_B[i][cat],'fan_count':tmp_B[i]['fan_count'],'likes':tmp_B[i]['likes']+1}
...
def categorized_pages_score(solr, userA_id, userB_id,category):
    pagesA=solr.search(q='doc_type:page AND user_id:'+str(userA_id)+' AND type:'+str(category),rows=pow(10,6),wt='python')
    pagesB=solr.search(q='doc_type:page AND user_id:'+str(userB_id)+' AND type:'+str(category),rows=pow(10,6),wt='python')
    tmp_A=[]
    tmp_B=[]


    for i in pagesA:
        tmp_A.append({'page_id':i['page_id'],'name':i['name'],'genre':i['genre'],'category_list':i['category_list'],'fan_count':i['fan_count'][0]})

    for i in pagesB:
        tmp_B.append({'page_id':i['page_id'],'name':i['name'],'genre':i['genre'],'category_list':i['category_list'],'fan_count':i['fan_count'][0]})
